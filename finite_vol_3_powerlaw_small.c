...
// ---------------------------------------------------------------------
// Milestones 5 & 6: local shear rates + power-law (Ostwald-de Waele)
// shear-thinning viscosity, regularized near zero shear rate.
//
// This supersedes the Carreau version (finite_vol_3_carreau_small.c) as
// the working closure: same plain-channel geometry and BCs (no nozzle),
// same 2D finite-volume architecture, same E/W/N/S grid, same pressure
// Poisson + SOR solve -- only the viscosity closure eta(|D|) changed.
// What's new relative to the milestone-3 (constant-viscosity) baseline:
//   - cell-centered strain-rate tensor D_ij = 0.5*(dvi/dxj + dvj/dxi),
//     and the scalar invariant |D| = sqrt(D:D)  (paper convention, not
//     the rheology-standard sqrt(2D:D) -- see the report chapter's note
//     on this factor-of-sqrt(2) rescaling)
//   - eta(|D|) via the regularized power-law model:
//         D_eff = sqrt(|D|^2 + eps^2)          (avoids the n<1 singularity
//                                                at |D|=0, since eta ~
//                                                |D|^(n-1) -> infinity
//                                                there without it)
//         eta   = K * D_eff^(n-1)
//     with K the consistency index, n the flow behavior index (n<1:
//     pseudoplastic/shear-thinning, n>1: dilatant/shear-thickening,
//     n=1: Newtonian with eta=K=mu), and eps the regularization scale.
//   - the viscous term in the momentum equation is the full stress
//     divergence div(2*eta*D), not eta*laplacian(v) -- required because
//     eta varies in space, so the two forms are no longer equivalent.
//
// Discretization notes (unchanged from the Carreau version):
...
// Power-law (Ostwald-de Waele) model parameters, regularized near zero
// shear rate to avoid the n<1 singularity in eta = K*|D|^(n-1).
// PLACEHOLDER VALUES -- replace with fitted parameters for the actual
// adhesive before drawing quantitative conclusions. Setting
// POWERLAW_N=1 recovers the Newtonian (constant-viscosity) case with
// mu=POWERLAW_K, useful as a sanity check that this reduces back to the
...
void init_fields()
{
    // start at the regularized zero-shear viscosity everywhere
    double eta0 = POWERLAW_K * pow(POWERLAW_EPSILON, POWERLAW_N - 1.0);
...
// Milestone 5: local shear rate.  Milestone 6: power-law viscosity from it.
...
void compute_shear_and_viscosity()
{
    ...
    for (int i=0;i<Ntot;i++){
        ...
        // regularized D_eff avoids the n<1 singularity eta -> infinity at |D|=0
        double D_eff=sqrt(Dmag[i]*Dmag[i]+POWERLAW_EPSILON*POWERLAW_EPSILON);
        eta[i]=POWERLAW_K*pow(D_eff, POWERLAW_N-1.0);
    }
}
...
void update_velocity()
{
    for (int x=1;x<=Nx;x++){
        for (int y=1;y<=Ny;y++){
            ...
            // face viscosities (averaged from the cell-centered power-law eta)
